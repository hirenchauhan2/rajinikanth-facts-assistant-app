...
app.intent("tell_fact", (conv, { category }) => {
  ...
  const categoryResponse = responses.categories.find(
    c => c.category === category
  )
  const fact = randomPop(facts[categoryResponse.category])
  if (!fact) {
    const otherCategory = responses.categories.find(
      other => other !== categoryResponse
    )
...
exports.factsAboutRajinikanth = functions.https.onRequest(app)
